...
- Scope baca (monitoring) dan scope command dipisahkan di `PermitMonitoringAccess`. SO/Officer atau Manager Pemilik Wilayah yang memiliki scope `ORF` memperoleh scope baca lintas lokasi (`*`) untuk daftar/detail PTW, lampiran, paket cetak, history, dan Papan Operasi (termasuk status terminal), sama seperti Administrator. Perluasan ini hanya untuk read; task, command, dan `EnsureLocationScope` tetap memakai scope lokasi asli aktor. Dasarnya adalah Amendemen 1 `docs/decisions/PTW-WORKFLOW-BASELINE.md` (ACCEPTED 25 September 2026); jangan menambah role atau lokasi ke perluasan ini tanpa superseding record.
- Daftar PTW (`GET /api/v1/permits?search=&offset=&limit=`) menerapkan filter Sponsor/role dan scope baca di dalam query sebelum pencarian, `Count`, dan `Skip`/`Take` (limit 1-100, default 25), bukan di memori setelah `Take`; urutan `UpdatedAt` lalu `Id` menurun agar halaman stabil. Parameter `search` dinormalisasi dan dibatasi 100 karakter di `PermitService`, di-escape sebagai pola `LIKE` di `PermitStore`, dan mencocokkan nomor PTW, `LocationId`, serta isi `DraftJson` (judul, perusahaan, dan field draft lain). Jangan menambahkan filter atau pencarian yang melewati scope ini.
