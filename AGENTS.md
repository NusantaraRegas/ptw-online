...
- Akun lokal, login cookie HTTP-only, dan `UserAuthorizationApproval:AllowAdministratorSelfApproval=true` hanya untuk environment `Development`. Role dan scope dihitung ulang dari assignment approved/effective pada setiap request; cookie dan klien bukan authority. Login memeriksa kredensial melalui Portal API (`IDirectoryAuthenticator` → `POST /api/v1/User/SecureAuth`, yang melakukan bind Active Directory) lebih dulu lalu fallback ke password lokal; kedua jalur mewajibkan akun terdaftar dan aktif, dan Portal API tidak memprovisikan user, role, atau scope. Token JWT dari Portal dibuang dan tidak pernah dipakai sebagai authority.
