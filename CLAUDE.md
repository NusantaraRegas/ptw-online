...
- [ ] Tidak ada kebijakan backlog yang dikarang: pejabat pengganti/delegasi, SLA dan eskalasi, ambang/umur gas test digital, periode retensi, angka RPO/RTO, klasifikasi data, kontrak API E-SIMI, scope perusahaan Kontraktor, QR. Tanpa decision record, jalur tersebut fail-closed. Keadaan sistem dan alur saat ini disahkan stakeholder 25 September 2026 (PTW-WORKFLOW-BASELINE Amendemen 2) bersama OPN-007, OPN-009, dan PROD-UPLOAD-SCAN; OPN-001 sampai OPN-006, OPN-010, dan OPN-011 ACCEPTED sejauh keadaan sistem menjawabnya (tiga area root tanpa hierarchy, route tunggal per lokasi tanpa matriks risiko, katalog statis sebagai master checklist, gas test hardcopy, urutan review, acknowledgement lewat Sponsor, tujuh hari tanpa batas renewal berantai, tanpa materi kampanye), OPN-008 dan OPN-012 sebagian. Ikuti record tersebut, jangan memperluasnya, dan jangan menggambarkan alur yang disahkan sebagai menunggu UAT atau sign-off. Klasifikasi header HOT/COLD hanya merepresentasikan checklist formulir (dan memetakan `RiskLevel` legacy pada COLD), bukan matriks routing risiko OPN-002.
...
- [ ] Scope baca dipisahkan dari scope command lewat `PermitMonitoringAccess`: SO/Officer atau Manager Pemilik Wilayah berscope `ORF` membaca PTW lintas lokasi (daftar, detail, lampiran, paket cetak, history, Papan Operasi dengan seluruh status non-draft) seperti Administrator, tetapi task pool, command, dan `EnsureLocationScope` tetap memakai scope lokasi asli. Dasarnya Amendemen 1 PTW-WORKFLOW-BASELINE (ACCEPTED 25 Sep 2026); perluasan role/lokasi lain memerlukan superseding record.
- [ ] Daftar dan pencarian PTW (`GET /api/v1/permits?search=&offset=&limit=`) menerapkan filter Sponsor/role dan scope baca (`PermitMonitoringAccess.ReadLocationScopes`) di query sebelum `Count` dan `Skip`/`Take` (limit 1-100, default 25; urutan `UpdatedAt`, `Id` menurun); `search` maksimum 100 karakter, di-escape sebagai pola `LIKE`, dan hanya mencocokkan nomor PTW, `LocationId`, dan `DraftJson`. Filter scope tidak boleh pindah ke memori atau ke UI.
...
- [ ] `POST /api/v1/auth/login` di luar `Development` hanya aktif dengan `Authentication:LoginEnabled=true` (`LoginSettings`), dan startup gagal bila nilai itu aktif tanpa `PortalAuth` https (OPN-007). Cookie sesi wajib `Secure` di luar `Development`.
- [ ] Setiap percobaan login dijurnal append-only ke `sec.LoginAuditEvent` (`ILoginAuditStore`) dengan alamat sumber, hasil Portal, jalur identitas, dan hasil; password tidak pernah dicatat dan jurnal hanya dibaca Administrator (`GET /api/v1/admin/users/login-events`). Lockout lokal (`lockout_triggered`/`locked_out`) dicatat sebagai warning `LocalAccountLockedOut` dengan alamat sumber, tetapi respons login tetap `401 authentication.invalid_credentials`.
- [ ] Cookie sesi membawa claim `security_stamp` yang harus sama dengan `UserAccount.SecurityStamp`; reset password, penonaktifan, dan `POST /api/v1/admin/users/{subjectId}/sessions/revoke` (`If-Match`) memutar stamp. Jangan menambahkan jalur yang mengubah kredensial atau status akun tanpa memutar stamp.
- [ ] Password lokal: 12-128 karakter, tiga kelas karakter, tidak memuat username, dan lolos `IBreachedPasswordChecker` (deny-list offline selalu aktif; range API k-anonymity hanya dengan `PasswordPolicy:BreachCheck:Enabled=true` dan https). Layanan tidak tersedia berarti password ditolak (`503`), bukan diloloskan.
- [ ] Di luar `Development`, `Authentication:DataProtectionPath` mewajibkan `Authentication:DataProtectionCertificatePath` (PFX di luar repository) sehingga key ring tersimpan terenkripsi; jangan melonggarkan `DataProtectionKeyCertificate` menjadi opsional. Batas body Kestrel harus sama dengan `PTW_UPLOAD_MAX_BODY` nginx.
- [ ] CSP memakai nonce per request untuk `style-src` (`$request_id` di nginx, placeholder `ngCspNonce` pada `index.html`); jangan mengembalikan `'unsafe-inline'` dan jangan menambahkan `<style>`/`<script>` inline tanpa nonce. `ForwardedHeaders:KnownNetworks`/`KnownProxies` membatasi `X-Forwarded-*` ke subnet nginx; rate limiter in-process dikunci per subject id atau alamat klien (`RateLimitSettings`), dengan policy `login` yang lebih ketat; nginx (`prod.conf.template`) adalah limiter utama. Respons `/api/*` membawa `Cache-Control: no-store`.
...
- [ ] Konfigurasi `PortalAuth:*` tidak memuat kredensial service account (hanya meneruskan kredensial pengguna ke `SecureAuth`); `PortalAuth:Enabled` tidak ditaruh di `appsettings.json` dasar; `BaseUrl` ber-skema `http://` hanya diterima pada Development dan menggagalkan startup di luar itu, kecuali opt-in literal `PortalAuth:AllowInsecureHttp=true` (`PTW_PORTAL_API_ALLOW_INSECURE_HTTP`) sesuai amendemen OPN-007 ACCEPTED 24 September 2026 untuk Portal `http://10.10.10.12:7100` di jaringan internal; api mencatat warning `InsecurePortalHttpAccepted` setiap start. Jangan memperluas opt-in ini ke skema, host, atau konteks lain. Service `api` di compose tetap berada di network `frontend` (dev: `extra_hosts` ke `host-gateway`) agar panggilan Portal keluar tidak diam-diam gagal ke password lokal.
