...
- [ ] Kode kontrak API (`HotWork`, `USER_SPONSOR`, kode lokasi seperti `ORF`, kode katalog) tidak ditampilkan mentah; UI memetakannya ke label Bahasa Indonesia (`Pekerjaan Panas`, `Pekerjaan Dingin`, `Memasuki Ruang Terbatas`, `User Sponsor`, `Kontraktor`). Field opsional seperti Work Order No. memberi teks bantuan yang terasosiasi lewat `aria-describedby`.
