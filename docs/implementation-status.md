...
Tanggal pemeriksaan: 23 September 2026. Dokumen requirement v1.8 diperlakukan sebagai sumber kebutuhan; keputusan OPN-001–012 yang belum disahkan tidak diberi nilai bisnis fiktif.

BRD/PRD/FSD v1.8 (22 September 2026) menggantikan v1.7 dan menyelaraskan requirement dengan flow yang sudah berjalan: review Bagian 7 oleh SO/Officer pemilik wilayah sebelum approval Manager, pembagian pengisian Bagian 1–7 (Sponsor 1–4, PIC HSE 5, SO/Officer dan Manager 7), dokumen dasar JSA/ID/BPJS TK/FTW/E-SIMI, renewal berbasis review pemilik wilayah, closure dengan verifikasi Bagian 10, notifikasi revisi Sponsor, paket cetak dua halaman A3, dan identitas lokal Development. Implementasi saat ini juga mewajibkan upload Prosedur Pekerjaan sesuai permintaan 22 September 2026 tanpa otomatis memilih checkbox Bagian 4. Keputusan baru tercatat sebagai DEC-117 sampai DEC-126 pada BRD v1.8; ID requirement pada tabel di bawah mengacu pada penomoran v1.8.
...
| User lokal Development dan role dinamis | `UserAccount`, `UserDirectoryService`, `UserAuthenticationService` (AD-first, fallback lokal), `LdapDirectoryAuthenticator` (`AD:*`), `UserAuthorizationRoleProfiles`, cookie authentication, `UserDirectoryStore` | `/api/v1/auth/login`, `/logout`, `/admin/users`; assignment langsung menerima user/role/area/periode dan menurunkan action server-side; `/me` mengambil role/scope assignment approved/effective | `AddUserAccountsAndSignatures` | domain user invariant + API create/signature/login/controlled-role assignment + `DirectoryLoginApiTests` (AD sukses, AD tanpa akun terdaftar, akun nonaktif, fallback lokal saat AD menolak/tidak tersedia, gagal ganda) + Angular API/interceptor regression |
...
| Dokumen dasar wajib sebelum submit | `PermitMandatoryDocumentCatalog`, `PermitAttachmentService`, `PermitService` | `GET /reference-data/mandatory-documents`; JSA, Prosedur Pekerjaan, ID, BPJS TK, FTW, dan E-SIMI harus memiliki lampiran bertaut. Prosedur Pekerjaan memakai kode Bagian 4 yang sama, tetapi kewajiban upload tidak otomatis memilih checkbox cetak; empat evidence identitas/integrasi lainnya tidak mengubah checklist PDF Bagian 4 | n/a (memakai `SupportingDocumentCode` attachment yang tersedia) | domain separation regression, API per-document negative submit, Angular readiness/upload flow |
...
2. Sebagai Sponsor, buat draft ORF, Site-Office, dan Water-Based Activity. Pastikan JSA, Prosedur Pekerjaan, ID, BPJS TK, FTW, dan E-SIMI tampil sebagai dokumen dasar wajib. Pilih beberapa dokumen Bagian 4 opsional, unggah semua file dengan jenis yang sesuai, lalu submit; tanpa salah satu dokumen dasar, metadata JSA yang cocok, atau salah satu file tambahan terpilih, submit harus ditolak. Setelah lengkap, pastikan masing-masing hanya membuat satu task `HSE_VALIDATION`.
